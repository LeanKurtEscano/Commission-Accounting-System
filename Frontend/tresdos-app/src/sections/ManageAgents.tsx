...
const ManageAgents = () => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>,  agentType: string,
    agentName: string,
    agentId: number,
    parent:number | null,
    parentId: number | null) => {
      ...
      setFormData((prev) => {
        ...
        const existingIndex = prev.findIndex((agent) => agent.id === agentId && agent.type === agentType);
                      ...
                      <div className="flex items-center space-x-4 mb-2">
                        ...
                        <p className="text-slate-300 mr-2">Commission: </p>
                          {edit.id === middle.id && edit.agentType === "midAgent" ? (
                             ...
                             type="number"
                             value={
                               formData.find((item) => item.id === head.id && item.type === "midAgent")?.commission}
                             onChange={(e) => handleInputChange(e,"headAgent",head.name,head.id,null,null)}
                             className="w-14 p-2 rounded-md bg-darkbg text-slate-100 border border-textHeading focus:outline-none focus:ring focus:ring-textHeading"
                           />
